...
import {
  IFormUpdateDepartment,
 DepartmentExport,
  initialDepartmentUpdate,
  initialStateData,
} from "./interfaces";
...
export function Departments() {
  ...
  const [department, setDepartment] = useState<IFormUpdateDepartment>(
    initialDepartmentUpdate
  );
  ...
  const handleUpdate = (item: IFormUpdateDepartment) => {
    setDepartment(item);
    setOpenUpdate(!openUpdate);
    setSelectedRow(item.id);
  };

  const handleOpenDelete = (item: IFormUpdateDepartment) => {
    setDepartment(item);
    ...
    const handleExport = useCallback(async () => {
      try {
        ...
        if (response.status === 200) {
          const parseData = response.data.map((item: DepartmentExport) => ({
            Código: item.code,
            Descrição: item.description,
            "Data de criação": formatTime(item.createdAt),
          }));
          ExportXLSX(parseData, "Lista de Departamento");
        }
      } catch (error) {
        setAlert({
          open: true,
  ...
  //Inport file
    const handleUploadDepartment = async (
      ...
      const file = files[0];
      try {
        const response = await uploadDepartment(file);
        if (response && (response.status === 201 || response.status === 200)) {
          setAlert({
            open: true,
            message: "Upload de departamento realizado com sucesso.",
            type: "success",
          });
          setDataRefresh(true);
        }
  ...
  const fetchData = async (page: number) => {
    try {
      const response = await findManyDepartment(page);
      setData({
  ...
  const searchData = useCallback(
    async (page: number) => {
      try {
        const response = await searchForDepartment(page, searchValue);
        setData({
          departments: response.data.departments,
          total: response.data.total,
          currentPage: response.data.currentPage,
            ...
            {data.departments.map((department) => {
              return (
                <TableRow
                  ...
                  sx={{
                    backgroundColor:
                      selectedRow === department.id ? COLORS.PRIMARY_50 : "inherit",
                    "&:hover": {
